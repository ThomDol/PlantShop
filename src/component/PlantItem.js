...
const PlantItem = ({ name, cover, category, id, water, light }) => {
    ...
    return (
        ...
        <div key={id} className="card" style={{ width: '18rem' }}>
            <img onClick={displayUrl} src={`${cover}`} className="card-img-top" alt={`${name} cover`} />
            <div class="card-body">
                <h5 class="card-title">{name}</h5>
                <p><span className='bg-secondary border rounded p-1' style={{ width: 'auto', height: 'auto', fontSize: '10px' }}>{category}</span></p>
